...
เพดานทั้งสองตัวถูกตรวจใน middleware ตัวเดียวกับที่เขียน log ไม่ได้ใช้
`extension.FixedComplexityLimit` เพราะ extension นั้นรันก่อน middleware ทุกตัว
query ที่ถูกมันปฏิเสธจะไม่มีบรรทัดไหนใน log เลย — ของที่โดนกันคือของที่อยากเห็น
มากที่สุด ทุกบรรทัดของ `graphql_operation` จึงมี `depth` กับ `complexity` ติดมาด้วย
แม้ query จะผ่าน ใช้ปรับเพดานจากข้อมูลจริงแทนการเดา
